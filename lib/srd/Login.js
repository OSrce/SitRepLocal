...
/*			var i;
			...
			console.log("LocalStorage Count="+myservers_count);
			if ( myservers_count > 0) {
				for(var i=0;i<myservers_count; i++) {
					var tempStr = localStorage["myservers_"+i];
					console.log("LocalStorage Server:"+i+ " Value:"+tempStr);
					if( !!tempStr ) {
						this.myservers[i] = dojo.fromJson( tempStr );
					}
				}
			}
*/
...
/*			var myservers_count = 0;
			myservers_count = localStorage["myservers_count"];
			console.log(" LocalStorage Count="+myservers_count);
			if ( myservers_count != null) {
				for(var i=0;i<myservers_count; i++) {
					delete localStorage["myservers_"+i];
				}
			}
